...
config_file: "$COW_WORKSPACE/fitness_coach/config.json"
runtime_data_dir: "$COW_WORKSPACE/fitness_coach"
...
- 长期数据保存在 `$COW_WORKSPACE/fitness_coach`：`profile.md` 存基础档案，`data/daily/` 存每日记录，`data/summaries/` 存周/月摘要，`config.json` 存提醒时间、必填字段和版本检查地址。
